...
For the live demo, there's a self-contained interactive HTML dashboard —
no Python, no server, just open it in a browser:

```bash
python3 export_dashboard_data.py   # regenerate outputs/dashboard_data.json from current data
python3 build_dashboard.py          # bake it into outputs/dashboard.html
# then just open outputs/dashboard.html in any browser
```
...
- **Phase 1 · Diagnose** — drag the ε slider to live-scrub the quality threshold; watch edges
  light up/fade and β₀ update in real time, while β₁ stays provably at 0
- **Phase 2 · Heal** — toggle "Show healed links" to overlay the 5 discovered missing links
  in red on the network, plus the persistence diagram, barcode, and full stats per candidate
